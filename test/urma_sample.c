 ...
 *
 * 4) 数据路径：
 *    - client_write_read: 演示 RDMA Write + Read（一侧直接读写另一侧内存）。
 *    - client_send + server_jetty_thread_main: 演示 Send/Recv + server 回包。
 *
 * 5) server 线程模型：
 *    - server_sock_thread_main: 接收新连接并交换元信息；
 *    - server_jetty_thread_main: 处理收发完成队列、复投 recv buffer、按需回复；
 *    - server_watch_thread_main: 周期打印共享段内容，便于观察。
 */
...
{
    /* server 线程1：accept + 元信息交换 + 导入对端 jetty */
    int fd = -1;
...
{
    /* client 主流程：init -> prepare -> write/read -> send -> destroy */
    int ret;
